...
File.prototype.substituteMappings = function (filepath) {
    ...
    var sep = (~filepath.indexOf('/')) ? '/' : '\\',
        ...
        first = segments[0],
        paths, mapping, mapName;
    // exit early if there's nothing to do
    if (!(paths = this.opts.paths) || filepath.charAt(0) !== '@') {
        return filepath;
    }
    // strip the @ symbol from the first segment to get the alias name
    mapName = first.substr(1);
    // check to see if there's a path mapping for first path segment
    if (mapping = paths[mapName]) {
        // Don't deal with baseUrl refs here, because those are handled in the 
