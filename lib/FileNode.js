...
function splitToGlobOrTextNode(node, text) {
	...
	var baseOpt = node.file.opts.baseUrl;
	...
	if (matchPath) {
		...
		if (matchPath.substr(0, 5) === ':base') {
			...
			// baseUrl instead.
			basePath = (baseOpt) ? path.resolve(process.cwd(), baseOpt) : process.cwd();
			...
			matchPath = matchPath.replace(':base', '.');
		}
