'use strict';

module.exports = GlobNode;

var glob = require('glob'),
	q = require('q'),
	FileNode = require('./FileNode');

function GlobNode(globpath, opts) {
	this.opts = opts;
	this.globpath = globpath;
	this.expand();
}

GlobNode.prototype.expand = function () {
	var self = this,
		promise = q.defer();

	this.promise = promise.promise;

	glob(this.globpath, function (err, files) {
		self.nodes = files.filter(function (filepath) {
			return !self.opts.seen[filepath];
		}).map(function (filepath) {
			return new FileNode(filepath, self.opts);
		});

		if (!self.nodes.length) {
			if (typeof self.opts.debug === 'function') {
				self.opts.debug('No files matched "' + self.globpath + '"');
			} else if (self.opts.debug) {
				console.log('No files matched "' + self.globpath + '"');
			}
			promise.resolve();
			return;
		}

		q.all(self.nodes.map(function (node) {
			return node.promise;
		})).done(function () {
			promise.resolve(self.nodes);
		});
	});
};

GlobNode.prototype.toString = function () {
	var separator = this.opts.separator || '\n';

	return this.nodes.map(function (node) {
		return node.toString();
	}).join(separator);
};
